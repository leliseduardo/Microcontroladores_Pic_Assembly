;
;		O objetivo deste programa é implementar um voltimetro a partir da leitura do conversor AD. Para isso, necessita-se
;	utilizar as operações de multiplicação e divisão, que serão obtidas da nota de aplicação AN544, feira pela microchip.
;		No circuito deste programa, pode-se utilizar um divisor de tensão que receba, por exemplo, até 25V e entregue de
; 0V a 5V na entrada analógica do do Pic. Assim, mede-se indiretamente a tensão de 0V a 25V.
;		Para fazer a conversão do valor lido no ADC em tensão, basta fazer uma regra de 3:
;
;		255 ----------  25V
;		ADC ---------- Tensão
;			Tensão = ( ADC * 25V) / 255
...
;	caso se utilize um display, seja colocado um ponto para representar as casas decimais. Logo, multiplicando a tensão
;	por 100, a equação fica
;	
;			Tensão = (ADC * 250) / 255
;
;		Para fazer uma equação desta em Assembly, necessita-se criar as funções de multiplicação e divisão, trabalho já
;	realizado pela Microchip em sua nota de aplicação AN544.
;
